document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('newDate').addEventListener('change', async function() {
        ...
        if (selectedDate && currentAppointment) {
            try {
                const response = await fetch(`/cit17-final-project/public/api/available-timeslots?date=${selectedDate}&appointment_id=${currentAppointment.appointment_id}`);
                if (!response.ok) {
                ...
                if (data.success) {
                    ...
                    throw new Error(data.message || 'Failed to load available time slots');
                }
            } catch (error) {
                console.error('Error:', error);
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error.message || 'Failed to load available time slots'
                });
            }
        }
    });
